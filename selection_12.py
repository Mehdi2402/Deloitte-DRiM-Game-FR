# -*- coding: utf-8 -*-
"""
Created on Sat Nov 28 12:20:30 2020
...
# Importer la base
mat12 = pd.read_csv(r'C:\Users\nelso\OneDrive\Bureau\mat12.csv', sep=';' )

#Drop des variables non explicatives
mat12 = mat12.drop(['cle2', 'dte_def_2', 'dte_sor_2', 'dtf_per_trt', '_maturity_', '_clos_',
...
mat12=mat12[mat12["MT_INI_FIN_"]!=0]

#Etude des corrélations
QuantVar=mat12[[col for col in mat12.columns.to_list() if mat12[col].nunique() > 6]]
quantfeat=corrcont(QuantVar)


#####To drop après études des corrélations
mat12=mat12.drop(columns=["ead","dur_b_defm_","mt_appo_","dur_b_endm_",
   ...
   "cpi_spain", "tic_spain", "MT_INI_FIN__qual_veh",\
      "DUR_PREV_FIN_cat_seg"],axis=1)
mat12=mat12[mat12["outliers"]==0]
mat12=mat12.drop(["tweaked_maturity","outliers"], axis=1)
######

'''
bin_values = np.arange(start=0.01, stop=1, step=0.05)
mat12["tx_rec_marg"].hist(bins=bin_values, figsize=[14,6])

mat12["tx_rec_marg"].quantile([0,0.25,0.5,0.75,1])
'''

#Dicrétisation de la target
median_bin = list(mat12[mat12["tx_rec_marg"]> 0.05]["tx_rec_marg"].describe())[5]
mat12['tx_rec_marg_Bin'] = pd.cut(mat12['tx_rec_marg'] , [-1,0.05,median_bin,1.5] , labels = [0,1,2])
mat12['tx_rec_marg_Bin'] =  mat12['tx_rec_marg_Bin'].astype(float)
...
ToStandardize=mat12.drop([c for c in mat12.columns if c not in Standardize], axis=1)
mat12=mat12.drop([c for c in mat12.columns if c in Standardize], axis=1)
...
Tokeep=list(list(mat12.columns) + list(ToStandardize.columns))
mat12=pd.concat([mat12.reset_index(drop=True),ToStandardize_scaled.reset_index(drop=True)],axis=1,ignore_index=True)
mat12.columns=Tokeep

X = mat12.drop(["tx_rec_marg","tx_rec_marg_Bin"], axis=1)
y = mat12['tx_rec_marg_Bin']
...
def RFECVRF(X_train,y_train,NumberOfStep,Kfold):
    ...
    clf_rf=LogisticRegression(multi_class='multinomial', solver='lbfgs',\
...
X_train, X_test, FeatList= RFERF(X,X_train, y_train,X_test,1,8)
...
mat12=mat12[modelfeat]
mat12.to_csv(r"C:\Users\nelso\OneDrive\Bureau\mat12nodiscr.csv")