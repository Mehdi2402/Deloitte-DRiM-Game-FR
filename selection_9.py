# -*- coding: utf-8 -*-
"""
Created on Sat Nov 28 10:51:33 2020
...
# Importer la base
mat9 = pd.read_csv(r'C:\Users\nelso\OneDrive\Bureau\mat9.csv', sep=';' )

#Drop des variables non explicatives
mat9 = mat9.drop(['cle2', 'dte_def_2', 'dte_sor_2', 'dtf_per_trt', '_maturity_', '_clos_',
...
mat9=mat9[mat9["MT_INI_FIN_"]!=0]

####Etude des corrélations
QuantVar=mat9[[col for col in mat9.columns.to_list() if mat9[col].nunique() > 6]]

corrcont(QuantVar)


###############To drop après étude des corrélations
mat9=mat9.drop(columns=["ead","dur_b_defm_","mt_appo_","dur_b_endm_",
            ...
            "DUR_PREV_FIN_cat_seg","ead_CD_CAT_EXPO_4","DUR_PREV_FIN_CD_CAT_EXPO_4"],axis=1)

mat9=mat9[mat9["outliers"]==0]
mat9=mat9.drop(["tweaked_maturity","outliers"], axis=1)
################	

'''
bin_values = np.arange(start=0.01, stop=1, step=0.05)
mat9["tx_rec_marg"].hist(bins=bin_values, figsize=[14,6])

mat9["tx_rec_marg"].quantile([0,0.25,0.5,0.75,1])
'''

#Discrétisation de la target
mat9=mat9[mat9["tx_rec_marg"] >= 0]
median_bin = list(mat9[mat9["tx_rec_marg"]> 0.05]["tx_rec_marg"].describe())[5]
mat9['tx_rec_marg_Bin'] = pd.cut(mat9['tx_rec_marg'] , [-1,0.05,median_bin,1] , labels = [0,1,2])
mat9['tx_rec_marg_Bin'] =  mat9['tx_rec_marg_Bin'].astype(float)


#Standardisation des variables numériques
Standardize=['DUR_PREV_FIN', 'MT_INI_FIN_', 'mt_appo__cat_seg', 'mt_appo__CD_CAT_EXPO_4',\
             'DUR_PREV_FIN_cat_seg','ead_qual_veh',"ead_cat_seg",\
                 "chom_spain","cli_spain"]
    
ToStandardize=mat9.drop([c for c in mat9.columns if c not in Standardize], axis=1)
mat9=mat9.drop([c for c in mat9.columns if c in Standardize], axis=1)
...
Tokeep=list(list(mat9.columns) + list(ToStandardize.columns))
mat9=pd.concat([mat9.reset_index(drop=True),ToStandardize_scaled.reset_index(drop=True)],axis=1,ignore_index=True)
mat9.columns=Tokeep


X = mat9.drop(["tx_rec_marg","tx_rec_marg_Bin"], axis=1)
y = mat9['tx_rec_marg_Bin']
...
def RFECVRF(X_train,y_train,NumberOfStep,Kfold):
    ...
    clf_rf=LogisticRegression(fit_intercept=True,multi_class='multinomial',class_weight='balanced',solver='lbfgs',max_iter=300)
...
mat9.to_csv(r"C:\Users\nelso\OneDrive\Bureau\mat9nodiscr.csv")
