# -*- coding: utf-8 -*-
"""
Created on Sun Nov 29 17:45:30 2020
...
# Importer la base
mat24 = pd.read_csv(r'C:\Users\nelso\OneDrive\Bureau\mat24.csv', sep=';' )

#Drop des variables non explicatives
mat24 = mat24.drop(['cle2', 'dte_def_2', 'dte_sor_2', 'dtf_per_trt', '_maturity_', '_clos_',
...
mat24=mat24[mat24["MT_INI_FIN_"]!=0]


####Matrice corrélation
QuantVar=mat24[[col for col in mat24.columns.to_list() if mat24[col].nunique() > 6]]

corrcont(QuantVar)


###############To drop
mat24=mat24.drop(columns=["ead","dur_b_defm_","mt_appo_","dur_b_endm_",
...
mat24=mat24[mat24["outliers"]==0]
mat24=mat24.drop(["tweaked_maturity","outliers"], axis=1)
################	


'''
bin_values = np.arange(start=0.01, stop=1.01, step=0.05)
mat24["tx_rec_marg"].hist(bins=bin_values, figsize=[14,6])

mat24["tx_rec_marg"].quantile([0,0.25,0.5,0.75,1])
'''

#Discrétisation de la target
median_bin = list(mat24[mat24["tx_rec_marg"]>0.05]["tx_rec_marg"].describe())[5]
mat24['tx_rec_marg_Bin'] = pd.cut(mat24['tx_rec_marg'] , [-1,0.05,median_bin,1] , labels = [0,1,2])
mat24['tx_rec_marg_Bin'] =  mat24['tx_rec_marg_Bin'].astype(float)


#Standardisation des variables quantitatives
Standardize=['DUR_PREV_FIN', 'MT_INI_FIN_', 'mt_appo__cat_seg', 'mt_appo__CD_CAT_EXPO_4',\
             'DUR_PREV_FIN_CD_CAT_EXPO_4',"ead_cat_seg",'ead_qual_veh',"chom_spain",
             "cli_spain",'MT_INI_FIN__CD_CAT_EXPO_4']
    

ToStandardize=mat24.drop([c for c in mat24.columns if c not in Standardize], axis=1)
mat24=mat24.drop([c for c in mat24.columns if c in Standardize], axis=1)
...
Tokeep=list(list(mat24.columns) + list(ToStandardize.columns))
mat24=pd.concat([mat24.reset_index(drop=True),ToStandardize_scaled.reset_index(drop=True)],axis=1,ignore_index=True)
mat24.columns=Tokeep


#Découpage de la base
X = mat24.drop(["tx_rec_marg","tx_rec_marg_Bin"], axis=1)
y = mat24['tx_rec_marg_Bin']
...
X_train, X_test, FeatList= RFERF(X,X_train, y_train,X_test,1,12)
...
mat24=mat24[modelfeat]

mat24.to_csv(r"C:\Users\nelso\OneDrive\Bureau\mat24nodiscr.csv")